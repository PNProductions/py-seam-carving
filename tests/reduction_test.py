...
from seammerging.reduction import seam_carving_decomposition
...
S = X[:, :, 0] + 25
T = X[:, :, 0] + 50
subject = seam_carving_decomposition(X, S, T, 2, 0, 0.5, 0.5)


class seam_carving_decompositionTest(unittest.TestCase):
  def test_apply_seam_carving(self):
    p = scipy.io.loadmat('./tests/mats/apply_seam_carving.mat')
    I = p['I'].T[0] - 1
