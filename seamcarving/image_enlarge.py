...
class seam_carving_decomposition_enlargement(seam_carving_decomposition):
  ...
  def generateNorthEnergy(self, Simg, v, northA, northB, northC):
    ...
    DD = self.initD(Simg)
    # DD(2:end,2:end) = v(2:end,2:end) - Simg(1:end-1,2:end-1);
    DD[1:, 1:] = v[1:, 1:] - Simg[0:-1, 1:-1]
    CNcnCL = square(DD, northA, northB, northC)

    # Upper-right connection
    DD = self.initD(Simg)
    # DD(2:end,1:end-1) = v(2:end,1:end-1) - Simg(1:end-1,2:end-1);
  ...
  def generateSouthEnergy(self, Simg, v, southA, southB, southC):
    ...
    DD = self.initD(Simg)
    # DD(1:end-1,1:end-1) = v(1:end-1,1:end-1) - Simg(2:end,2:end-1);
    DD[0:-1, 0:-1] = v[0:-1, 0:-1] - Simg[1:, 1:-1]
    CScnCL = square(DD, southA, southB, southC)

    # Lower-right connection
    DD = self.initD(Simg)
    # DD(1:end-1,2:end) = v(1:end-1,2:end) - Simg(2:end,2:end-1);
